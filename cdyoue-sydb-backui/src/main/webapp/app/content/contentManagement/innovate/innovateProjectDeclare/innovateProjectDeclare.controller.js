(function () {
    ...
    angular
        .module('oddApp')
        .controller('InnovateProjectDeclareController', InnovateProjectDeclareController);

    InnovateProjectDeclareController.$inject = ['$scope','$uibModal', 'InnovateService','toaster'];

    function InnovateProjectDeclareController($scope, $uibModal,InnovateService,toaster) {
        var vm = this;
        vm.queryModel={
            sort:"-publishTime",
            pageSize: 10,
        ...
        vm.ProjectDeclarePage={};
        vm.dataWarning="";
        function init() {
            InnovateService.getProjectDeclare(vm.queryModel,function (data) {
                ...
                vm.ProjectDeclarePage=data.response;
                vm.dataShow = true;
            },function (err) {
                vm.dataShow = false;
                console.log(err)
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                init();
                vm.ProjectDeclarePage.pageNum = select;
        ...
        function openModal (index,model) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/innovate/innovateProjectDeclare/innovateProjectDeclareModal.html",
                controller : "InnovateProjectDeclareModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
                        if (index != null){
                            vm.passData.id=vm.ProjectDeclarePage.list[index].id;
                        } else {
                            vm.passData.id=null;
                        }
        ...
        function del(index) {
            vm.ids.push(vm.ProjectDeclarePage.list[index].id);
            var out = $uibModal.open({
                animation : true,
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    InnovateService.deleteProjectDeclare({ids:vm.ids}, function success(obj) {
        ...
        function checkedOne(id, index, checkOne) {
            ...
            if (vm.fLength == vm.ProjectDeclarePage.list.length) {
        ...
        function checkedAll(m) {
            vm.fLength = 0;
            for (var i = 0, len = vm.ProjectDeclarePage.list.length; i < len; i++) {
                //根据全选状态进行赋值
                if (m) {
                    vm.ids.push(vm.ProjectDeclarePage.list[i].id);
                    var BL = vm.ProjectDeclarePage.list[i].checked;
                    if (BL == false) {
                        vm.fLength += 1
                    }
                    vm.ProjectDeclarePage.list[i].checked = true;
                }
                else {
                    for (var y = 0; y < vm.ids.length; y++) {
                        if(vm.ids[y] == vm.ProjectDeclarePage.list[i].id){
                            vm.ids.splice(y,1);
                        }
                    }
                    vm.ProjectDeclarePage.list[i].checked = false;
                }
            }
        }
    }
