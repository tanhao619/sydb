(function () {
    ...
    angular
        .module('oddApp')
        .controller('ApplyTutorsController', ApplyTutorsController);

    ApplyTutorsController.$inject = ['toaster', '$uibModal', 'ConsultationService'];

    function ApplyTutorsController(toaster, $uibModal, ConsultationService) {
        var vm = this;
        ...
        init();
        function init() {
            ConsultationService.getApplyTutors(vm.queryModel, function success(obj) {
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                ConsultationService.getApplyTutors(vm.queryModel, function (rlt) {
        ...
        function selectPageSize(pageSize) {
            ...
            ConsultationService.getApplyTutors(vm.queryModel,function(data){
        ...
        function openModal (index) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/consultation/applyTutors/applyTutorsModal.html",
                controller : "ApplyTutorsModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
                        vm.passData.id=vm.syBanner.list[index].id;
                        vm.passData.createEnter = vm.syBanner.list[index].createEnter;
                        return vm.passData;
                    }
                }
            });
        ...
        function del(index) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    ConsultationService.delApplyTutor({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
