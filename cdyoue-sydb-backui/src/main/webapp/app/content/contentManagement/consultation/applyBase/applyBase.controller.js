(function () {
    ...
    angular
        .module('oddApp')
        .controller('ApplyBaseController', ApplyBaseController);

    ApplyBaseController.$inject = ['toaster', '$uibModal', 'ConsultationService'];

    function ApplyBaseController(toaster, $uibModal, ConsultationService) {
        var vm = this;
        ...
        init();
        function init() {
            ConsultationService.getApplyBases(vm.queryModel, function success(obj) {
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                ConsultationService.getApplyBases(vm.queryModel, function (rlt) {
        ...
        function selectPageSize(pageSize) {
            ...
            ConsultationService.getApplyBases(vm.queryModel,function(data){
        ...
        function openModal (index) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/consultation/applyBase/applyBaseModal.html",
                controller : "ApplyBaseModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
        ...
        function del(index) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    ConsultationService.delApplyBase({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
