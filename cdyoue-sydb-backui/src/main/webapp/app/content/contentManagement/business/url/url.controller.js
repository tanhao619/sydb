(function () {
    ...
    UrlController.$inject = ['toaster', '$uibModal', 'BusinessService'];

    function UrlController(toaster, $uibModal, BusinessService) {
        var vm = this;
        ...
        init();
        function init() {
            BusinessService.getUrls(vm.queryModel, function success(obj) {
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                BusinessService.getUrls(vm.queryModel, function (rlt) {
        ...
        function selectPageSize(pageSize) {
            ...
            BusinessService.getUrls(vm.queryModel,function(data){
        ...
        function openModal (index,model) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/business/url/urlModal.html",
        ...
        function del(index) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    BusinessService.delUrl({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
