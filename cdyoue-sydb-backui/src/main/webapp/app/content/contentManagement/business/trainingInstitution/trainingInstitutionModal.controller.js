(function () {
    ...
    angular
        .module('oddApp')
        .controller('TrainingInstitutionModalController', TrainingInstitutionModalController);

    TrainingInstitutionModalController.$inject = ['SYDB','$scope','$timeout', '$uibModalInstance','items','BusinessService','Upload','toaster'];

    function TrainingInstitutionModalController(SYDB,$scope,$timeout, $uibModalInstance,items,BusinessService,Upload,toaster) {
        var vm = this;
        vm.cancel=cancel; // 关闭
        vm.editor=editor; // 编辑
        vm.save=save; // 保存
        ...
        function initSyBanner() {
            ...
            if (items.id==null){
                ...
                BusinessService.getTrainingDetail({id:items.id},function (rel) {
                    if (rel.status == 'SUCCESS'){
                        vm.TrainingDetail = rel.response;
                        vm.updateItems = angular.copy(vm.TrainingDetail);
        ...
        function save() {
            ...
            if (items.showModel == "editor"){
                BusinessService.updateTraining({'id':items.id},vm.updateItems, function success(obj) {
                ...
                BusinessService.createTraining(vm.updateItems, function success(obj) {
                    if (obj.status == 'SUCCESS') {
                        toaster.clear();
                        toaster.pop('success', obj.message);
                        cancel();
