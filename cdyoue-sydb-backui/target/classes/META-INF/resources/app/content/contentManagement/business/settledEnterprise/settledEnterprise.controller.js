(function () {
    ...
    SettledEnterpriseController.$inject = ['$scope','$uibModal', 'BusinessService','toaster'];

    function SettledEnterpriseController($scope, $uibModal,BusinessService,toaster) {
        var vm = this;
        ...
        vm.q=null;
        function init() {
            BusinessService.getEnterprise(vm.queryModel,function (data) {
                ...
                vm.EnterprisePage=data.response;
                vm.dataShow = true;
            },function (err) {
                vm.dataShow = false;
                console.log(err)
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                init();
                vm.EnterprisePage.pageNum = select;
            }
        };
        ...
        function openModal (index,model) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/business/settledEnterprise/settledEnterpriseModal.html",
                ...
                resolve : {
                    items : function() {
                        if (index != null){
                            vm.passData.id=vm.EnterprisePage.list[index].id;
                        } else {
                            vm.passData.id=null;
                        }
        ...
        function top($index) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/business/settledEnterprise/settledEnterpriseTopModal.html",
                controller : "settledEnterpriseTopModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
                        vm.topData.enterpriseId=vm.EnterprisePage.list[$index].id;
                        vm.topData.topImage=vm.EnterprisePage.list[$index].topImage;
                        return vm.topData;
                    }
                }
            });
        ...
        function cancelTop(id) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    BusinessService.topEnterprise({id: id,topImage:null}, function success(obj) {
        ...
        function del(index) {
            vm.ids.push(vm.EnterprisePage.list[index].id);
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/layouts/messageTips/messageTips.html",
                controller : "messageTipsController",
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    BusinessService.deleteEnterprise({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
        ...
        function checkedOne(id, index, checkOne) {
            ...
            if (vm.fLength == vm.EnterprisePage.list.length) {
        ...
        function checkedAll(m) {
            vm.fLength = 0;
            for (var i = 0, len = vm.EnterprisePage.list.length; i < len; i++) {
                //根据全选状态进行赋值
                if (m) {
                    vm.ids.push(vm.EnterprisePage.list[i].id);
                    var BL = vm.EnterprisePage.list[i].checked;
                    if (BL == false) {
                        vm.fLength += 1
                    }
                    vm.EnterprisePage.list[i].checked = true;
                }
                else {
                    for (var y = 0; y < vm.ids.length; y++) {
                        if(vm.ids[y] == vm.EnterprisePage.list[i].id){
                            vm.ids.splice(y,1);
                        }
                    }
                    vm.EnterprisePage.list[i].checked = false;
                }
            }
        }
    }
