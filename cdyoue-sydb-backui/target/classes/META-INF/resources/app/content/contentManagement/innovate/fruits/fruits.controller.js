(function () {
    ...
    angular
        .module('oddApp')
        .controller('FruitsController', FruitsController);

    FruitsController.$inject = ['toaster', '$uibModal', 'InnovateService'];

    function FruitsController(toaster, $uibModal, InnovateService) {
        var vm = this;
        ...
        init();
        function init() {
            InnovateService.getFruits(vm.queryModel, function success(obj) {
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                InnovateService.getFruits(vm.queryModel, function (rlt) {
        ...
        function selectPageSize(pageSize) {
            ...
            InnovateService.getFruits(vm.queryModel,function(data){
        ...
        function openModal (index,model) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/innovate/fruits/fruitsModal.html",
                controller : "FruitsModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
        ...
        function del(index) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    InnovateService.delFruit({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
        ...
        function top(index) {
            ...
            if (top == 0) {
                var out = $uibModal.open({
                    animation : true,
                    templateUrl : "app/content/contentManagement/innovate/fruits/topModal.html",
                    controller : "topFruitsModalController",
                    controllerAs:"vm",
                    size : "lg",
                    resolve : {
                        items : function() {
                ...
                out.result.then(function (result) {//result关闭是回传的值
                    if(result == 1){
                        InnovateService.topFruit({id:vm.syBanner.list[index].id, topImgUrl:''}, function success(obj) {
                            if (obj.status == 'SUCCESS') {
                                init();
                                toaster.clear();
                                toaster.pop('success', obj.message);
