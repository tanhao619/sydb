(function () {
    ...
    KnowledgeAssessController.$inject = ['$scope','$uibModal', 'ConsultationService','toaster'];

    function KnowledgeAssessController($scope, $uibModal,ConsultationService,toaster) {
        var vm = this;
        ...
        function openModal(id) {
            var out = $uibModal.open({
                animation: true,
                templateUrl: "app/content/contentManagement/consultation/knowledgeAssess/knowledgeAssessModal.html",
                controller: "KnowledgeAssessModalController",
                controllerAs: "vm",
                size: "lg",
                resolve: {
                    items : {id:id}
        ...
        function delActivity(id) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    ConsultationService.deleteAssess({"id": id}, function success(obj) {
        ...
        function dels() {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    ConsultationService.deleteAllAssess({ids:vm.ids}, function success(obj) {
                        if(obj.status == "SUCCESS"){
                            toaster.pop('info', obj.message);
                            init();
                        }
         ...
         */
        function queryData() {
            ConsultationService.getAssess(vm.questionsObj, getIntellectualsSuccess, getIntellectualsError);
        }
        ...
        function selectPage(select, current, max) {
            if (select != current) {
                ...
                ConsultationService.getAssess(vm.questionsObj, function (obj) {
