(function () {
    ...
    ExpertsController.$inject = ['$uibModal', 'InnovateService', 'toaster'];

    function ExpertsController($uibModal, InnovateService, toaster) {
        var vm = this;
        ...
        init();
        function init() {
            InnovateService.getExperts(vm.queryModel, function success(obj) {
        ...
        function selectPage(select, current, max) {
            if (select !== current) {
                ...
                InnovateService.getExperts(vm.queryModel, function (rlt) {
        ...
        function selectPageSize(pageSize) {
            ...
            InnovateService.getExperts(vm.queryModel,function(data){
        ...
        function openModal (index,model) {
            ...
            var out = $uibModal.open({
                animation : true,
                templateUrl : "app/content/contentManagement/innovate/experts/expertsModal.html",
                controller : "ExpertsModalController",
                controllerAs:"vm",
                size : "lg",
                resolve : {
                    items : function() {
        ...
        function del(index) {
            ...
            out.result.then(function (result) {//result关闭是回传的值
                if(result == 1){
                    InnovateService.delExpert({ids:vm.ids}, function success(obj) {
                        if (obj.status == 'SUCCESS') {
                            toaster.clear();
        ...
        function top(index) {
            ...
            if (top == 0) {
                var out = $uibModal.open({
                    animation : true,
                    templateUrl : "app/content/contentManagement/innovate/experts/topModal.html",
                    controller : "topExpertsModalController",
                    controllerAs:"vm",
                    size : "lg",
                    resolve : {
                        items : function() {
                ...
                out.result.then(function (result) {//result关闭是回传的值
                    if(result == 1){
                        InnovateService.topExpert({id:vm.syBanner.list[index].id, topImgUrl:''}, function success(obj) {
                            if (obj.status == 'SUCCESS') {
                                init();
                                toaster.clear();
                                toaster.pop('success', obj.message);
