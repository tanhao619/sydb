    ...
    function deleteFunc(){
        ...
        function  showDataModel(config){
            ...
            if( Object.prototype.toString.call(config.data) == "[object Array]"){
                ...
                str+="<span>数据删除后不能恢复，请确认是否删除！</span>";
                //})
            }else{
            ...
            str+="</div>"
                ...
                +"<button type='button' class='btn btn-primary btn-add'   id='deleteId'  data-dismiss='modal'>确认</button>"
                +"<button type='button' class='btn btn-warning btn-del' data-dismiss='modal'>取消</button>"
                +"</div></div></div></div>";
        ...
        function dynamicDeleteModel(config){
            // $("body").off("click", '.#deleteId');
            $("body").off("click", '#deleteId');//修改--多次调接口
